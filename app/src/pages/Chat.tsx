...
function renderAssistantHtml(text: string, msgId: string): string {
  ...
  const badge = (n: string) =>
    `<button type="button" class="citation-badge" data-citation="${n}" data-msg-id="${msgId}">[${n}]</button>`;
...
export function Chat() {
  ...
  // Listen for citation badge clicks via event delegation
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      e.preventDefault();
      e.stopPropagation();
      const num = parseInt(badge.dataset.citation || "0", 10);
      const msgId = badge.dataset.msgId || "";
      const rect = badge.getBoundingClientRect();
      setCitPopover((prev) =>
        prev?.msgId === msgId && prev?.index === num
          ? null
          : { msgId, index: num, rect },
      );
    };
    ...
    <div className="flex flex-1 min-h-0 w-full max-w-4xl mx-auto flex-col">
      ...
      {chatId && (
        <div className="flex justify-end px-4 pt-2">
          <div className="inline-flex items-center rounded-lg border border-border bg-surface shadow-sm">
            <Tooltip content={linkCopied ? "Copied!" : "Copy share link"}>
              <button
                onClick={async () => {
                  const result = await shareChat.mutateAsync(chatId);
                  if (result.shared && result.share_token) {
                    const url = `${window.location.origin}/shared/chat/${result.share_token}`;
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-muted hover:text-brand-blue hover:bg-brand-blue/5 transition-colors rounded-l-[7px]"
              >
                {linkCopied ? <CheckIcon className="h-3.5 w-3.5 text-green-500" /> : <LinkIcon className="h-3.5 w-3.5" />}
            ...
            <Tooltip content="Delete chat">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-muted hover:text-red-500 hover:bg-red-50 transition-colors rounded-r-[7px]"
              >
